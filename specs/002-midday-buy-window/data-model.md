# Data Model: Okno Najniższej Ceny Zakupu w Środku Dnia

## 1. QuarterHourPricePoint

**Purpose**: Normalized purchase-price sample for one quarter-hour slot from the configured current-day price-series source.
...
| `buy_price_value` | `float` | Parsed purchase price value used for comparisons |
...
**Relationships**
- Many `QuarterHourPricePoint` objects feed one `MiddayBuyWindowCandidate`.

## 2. MiddayBuyWindowCandidate
...
| `total_cost` | `float` | Sum of the 8 normalized purchase prices |
...
- `total_cost` is derived only from purchase-price values.
...
- The sensor must ignore sell-price state changes for both calculation and published output.
